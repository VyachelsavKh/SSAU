...
class LabelEntry(Element):
    def __init__(self, root : tk.Misc, text : str = "", entry_max_len : int = 15, entry_start_text : str = "", height = 20, font = ("Arial", 10), write_restrict : Callable[[str], str] = None):
        self.__root = root
        self.__entry_max_len = entry_max_len

        self.__write_restrict = write_restrict
    ...
    def __create_elements(self):
        ...
        self.__text_var.trace_add('write', self.__entry_update)
    ...
    def __entry_update(self, *args):
        value = self.__text_var.get()
        if len(value) > self.__entry_max_len:
            self.__text_var.set(value[:self.__entry_max_len])

        if self.__write_restrict is not None:
            value = self.__text_var.get()
            value = self.__write_restrict(value)
            self.__text_var.set(value)
